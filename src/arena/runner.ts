...
export async function playDecisionLoop(
    ...
    while (!view.hand_over && steps < MAX_STEPS_PER_HAND) {
        ...
        const desired = await seatAgents[seat].decide(game, view, seat);
    ...
    return { view, malformed, resolved };
}
...
export async function runHands(opts: RunOptions): Promise<void> {
    ...
    for (let hand = 0; hand < hands; hand++) {
        ...
        const { malformed, resolved } = await playDecisionLoop(env, first, agents, repo);
